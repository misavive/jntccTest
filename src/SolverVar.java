...
public  class SolverVar {
    ...
    public static void main(String[] args) {
        ...
        IntVar b = new IntVar(store, "b", 0, 8);
        IntVar c = new IntVar(store, "c", 0, 7);
        IntVar d = new IntVar(store, "d", 0, 6);
        IntVar e = new IntVar(store, "e", 0, 5);
        IntVar f = new IntVar(store, "f", 0, 4);
        IntVar g = new IntVar(store, "g", 0, 3);
        IntVar h = new IntVar(store, "h", 0, 2);
        IntVar i = new IntVar(store, "i", 0, 1);
        IntVar j = new IntVar(store, "j", 0, 6);
       ...
       // store.impose(new SumWeight (fghij, weights5, value_fghij));
        ...
        SelectChoicePoint select = new SimpleSelect (digits,
                new MostConstrainedStatic (), new IndomainRandom());
        ...
        label.getSolutionListener().setSolutionLimit(1000);
        ...
        if(result) {
            ...
            System.out.println("Solucion" +label.getSolutionListener().getSolution(10));

        }

        int numSolutions = label.getSolutionListener().solutionsNo();
        System.out.println("Number of Solutions: " + numSolutions);
        String [][] ltras = new String [numSolutions][variables.length];
        for(int s = 1; s <= numSolutions; s++) {
            Domain[] res = label.getSolutionListener().getSolution(s);
            int len = res.length;
            System.out.print("Solution #:" + s + " ");
            for(int x = 0; x < len; x++) {
                 ltras[s-1][x] = variables[Integer.valueOf(res[x].toString())];//.substring(10,cadena.length());
                System.out.println(res[x].toString()); //toStringConstraints()                 //+res[x].toStringFull()
            }
        ...
        for(int s = 0; s < numSolutions; s++) {
            for(int x = 0; x < variables.length; x++) {
                ...
                System.out.print(ltras[s][x]);
            }
            System.out.println("listando Arreglos");
        }
